...
const Sidebar = ({ open, onClose, user, onUsernameEdit }) => {
  ...
  const handleSupport = () => {
    onClose();
    // You can implement support logic here
    setTimeout(() => {
      ...
      <AnimatePresence>
        {open && (
          <motion.aside
            ...
            className="fixed top-0 left-0 h-screen w-4/5 max-w-sm bg-gradient-to-br from-slate-900 to-slate-950 backdrop-blur-xl z-40 flex flex-col border-r border-white/10 overflow-hidden"
          >
            {/* Header */}
                ...
                <div className="px-3 space-y-1">
                  {menuItems.map((item, i) => (
                    ...
                    >
                      ...
                      <span className="font-medium group-hover:text-blue-400 transition-colors text-sm">
                        {item.label}
                      </span>
                    </motion.div>
                  ))}
...
const Settings = () => {
  ...
  // Load saved user data and theme
  useEffect(() => {
  ...
  // Save user data and apply theme changes
  useEffect(() => {
